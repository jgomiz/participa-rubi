  ...
  def change
    ...
    weird_characters.each do |character|
      Decidim::UserBaseEntity.where(deleted_at: nil).where("name like '%#{character}%' escape '\' OR nickname like '%#{character}%' escape '\'").find_each do |entity|
        ...
        entity.name = if entity.name.size > 1
                        entity.name.delete(characters_to_remove).strip
                      else
                        entity.name.gsub(character, '_')
                      end
        sanitized_nickname = if entity.nickname.size > 1
                        entity.nickname.delete(characters_to_remove).strip
                      else
                        entity.nickname.gsub(character, '_')
                      end
